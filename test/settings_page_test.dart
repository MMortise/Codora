...
void main() {
  ...
  testWidgets('the image switch comes alive once an address is set',
    ...
    await openSettings(tester,
        settings: const AppSettings(
            v2exProxy: 'https://p.example.com/', v2exProxyImages: true));
  ...
  testWidgets('an address typed into the proxy field is saved', (tester) async {
    ...
    await tester.pumpAndSettle();
    await tester.enterText(field, '  https://p.example.com/  ');

    await tapAt(tester, saveFor('代理地址'));

    expect(container.read(settingsProvider).v2exProxy, 'https://p.example.com/');
