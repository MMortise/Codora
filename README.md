...
轨道底部的书签是「收藏和历史」：读帖子时点右上角的书签就收进来，读过的帖子按时间记在历史里，三个站混在一起列。

顶栏的放大镜搜索当前站点。linux.do 和掘金在应用里搜，结果替换当前板块的列表，点任意板块就回去；V2EX 没有搜索接口，搜索词交给必应（`site:v2ex.com/t`）在浏览器里打开。

有人回复或提到你时，应用不在前台会弹系统通知，程序坞图标上标出未读数（Windows 和 Linux 没有图标角标）。在「设置 → 常规 → 通知」里可以按站点关掉。这要求能读到这个站的消息：V2EX 填了 Token，linux.do 登录后。

快捷键：`J` / `K` 切换帖子，空格翻页，`⌘F` 或 `/` 搜索，`⌘1`–`⌘9` 切换站点；按 `?` 看全部。在回复框里打字时都不会触发。

在帖子卡片上右键可以屏蔽节点或作者，屏蔽的作者在帖子里的回复会折叠成一行，点「显示」再展开。
